...
while True:
    ...
    print(
        "EXCEPT THOSE DEALING WITH GREECE. WHAT KIND OF PROBLEMS DO YOU HAVE (ANSWER SEX, HEALTH, MONEY, OR JOB)?"
    )
    ...
    if problem == "SEX":
        ...
        if d_answer == "TOO MUCH":
            ...
            print(
                "WHY ARE YOU HERE? YOU SHOULD BE IN TOKYO OR NEW YORK OR AMSTERDAM OR SOMEPLACE WITH SOME REAL ACTION."
            )
        else:
            print(
                f"DON'T GET ALL SHOOK, JUST ANSWER THE QUESTION WITH 'TOO MUCH' OR 'TOO LITTLE'. WHICH IS IT?"
            )
            continue
    elif problem == "HEALTH":
        ...
        print(
            f"SORRY, {name}, I'M BROKE TOO. WHY DON'T YOU SELL ENCYCLOPEDIAS OR MARRY SOMEONE RICH OR STOP EATING SO YOU WON'T NEED SO MUCH MONEY?"
        )
    elif problem == "JOB":
        print(
            f"I CAN SYMPATHIZE WITH YOU, {name}. I HAVE TO WORK VERY LONG HOURS FOR NO PAY -- AND SOME OF MY BOSSES REALLY BEAT MY KEYBOARD."
        )
...
if g_answer == "YES":
    print(
        f"HEY, {name}??? YOU LEFT NO MONEY AT ALL! YOU ARE CHEATING ME OUT OF MY HARD-EARNED LIVING."
    )
    print("RIP OFF, *=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*")
elif g_answer == "NO":
    print(
        f"THAT'S HONEST, {name}, BUT HOW DO YOU EXPECT ME TO GO ON WITH MY PSYCHOLOGY STUDIES IF MY PATIENTS DON'T PAY THEIR BILLS?"
    )
    print("NOW LET ME TALK TO SOMEONE ELSE.")
else:
    print(
        f"YOUR ANSWER OF '{g_answer}' CONFUSES ME. PLEASE RESPOND WITH A 'YES' OR 'NO'."
    )
